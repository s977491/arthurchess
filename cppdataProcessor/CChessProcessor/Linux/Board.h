...
class Position {
public:
	Location locFrom, locTo, locLastFrom, locLastTo;
	bool win;
	int step;
	char position[MX_Y][MX_X];
	Position(char(*p)[MX_Y][MX_X], Location _locFrom, Location _locTo, Location _locLastFrom, Location _locLastTo, bool _win, int _step)
	: locFrom(_locFrom), locTo(_locTo), locLastFrom(_locLastFrom), locLastTo(_locLastTo), win(_win), step(_step)
	{
		copy(&(*p)[0][0], &(*p)[0][0] + MX_Y*MX_X, &position[0][0]);
	}
};
...
public:
	...
	string gameName;
	vector<shared_ptr<Position> > positionList;

	Board(string& _gameName) :  gameName(_gameName){
		resetBoard();
	}
	virtual ~Board() {
	}
	...
	void writeFile(const Location& from, const Location& to) {
		shared_ptr<Position> p(new Position(position, from, to, lastStepFrom, lastStepTo, winner == curSide, totalStep));
